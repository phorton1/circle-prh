...
void print_attr_list(int level, frame *frame);
void print_attr_lists(int level, struct frame *frm);
void print_de(int level, struct frame *frm, int *split, uint16_t *psm, uint8_t *channel);


void printAttributeList(u8 *p, u16 len, u16 cid)
{
    frame frm;
    frm.ptr = p;
    frm.len = len;
    ...
    frm.handle = 0;
    #if 1
        print_attr_lists(0, &frm)    ;
    #elif 1
        print_attr_list(0, &frm);
    #else
        int split;
        u16 psm;
        u8 channel;
        print_de(2, &frm, &split, &psm, &channel);
    #endif
}   
...
// u128 get_u64(frame *frm);

bool isprint(u8 c)  { return c >= ' '; }

void raw_dump(int level, frame *frame)
{
    display_bytes("raw_dump",frame->ptr,frame->len);
}
...
// u8   get_u8(frame *frm);
// u16  get_u16(frame *frm);
// u32  get_u32(frame *frm);
// u64  get_u64(frame *frm);
// void get_u128(frame *frm, u64 *val1, u64 *val2);
...
const char* get_uuid_name(int uuid)
{
	unsigned int i;
...
static inline const char* get_attr_id_name(int attr_id)
{
	unsigned int i;
...
static inline uint8_t parse_de_hdr(struct frame *frm, int *n)
{
    ...
    printf(" t(0x%02x=%d,%d,",de_hdr,de_type,siz_idx);
...
static inline void print_des(bool alt, uint8_t de_type, int level, int n, struct frame *frm, int *split, uint16_t *psm, uint8_t *channel)
{
	int len = frm->len;
    printf("%ssequence\n",alt?"alt ":"");
	while (len - (int) frm->len < n && (int) frm->len > 0)
		print_de(level+1, frm, split, psm, channel);
}
...
void print_de(int level, struct frame *frm, int *split, uint16_t *psm, uint8_t *channel)
{
	...
	switch (de_type)
    {
        ...
        case SDP_DE_STRING:
            ...
            break;
        case SDP_DE_SEQ:
            print_des(false,de_type, level, n, frm, split, psm, channel);
            return;
        case SDP_DE_ALT:
            print_des(true,de_type, level, n, frm, split, psm, channel);
            return;
	}
    printf("\n");
}
...
void print_attr_list(int level, struct frame *frm)
{
...
void print_attr_lists(int level, struct frame *frm)
{
    ...
    {
        printf("list of attributes lists\n");
...
static const char *pid2str(uint8_t pid)
{
	switch (pid) {
	case SDP_ERROR_RSP:
		return "Error Rsp";
