...
LCDWIKI_KBV::LCDWIKI_KBV(uint16_t model, uint8_t cs, uint8_t cd, uint8_t wr, uint8_t rd, uint8_t reset)
{
	LOG("LCDWIKI_KBV::LCDWIKI_KBV() ...",0);
		...
		// put a test pattern on pins 0..12
		// where every four pins counts to 16
		
		#if 0
			for (int i=0; i<10000; i++)
			{
				int bits = i & (0xf);
				int pattern = bits | (bits << 4) | (bits << 8) | (bits << 12);
				for (int j=0; j<13; j++)
				{
					digitalWrite(j,(pattern & (1<<j))?HIGH:LOW);
				}
    ...
    setWriteDir();
 	width = WIDTH;
	height = HEIGHT;
	LOG("LCDWIKI_KBV::LCDWIKI_KBV() finished",0);
}
...
{
	LOG("LCDWIKI_KBV::Init_LCD() called.  lcd_model=0x%04x\n",lcd_model);
	reset();
	
	#ifdef __circle__	// read the ID for sanity check
		lcd_model = 0xffff;
	#endif
...
void LCDWIKI_KBV::reset(void)
{
		...
		for (int i=0; i<2; i++)
		{
			for (int i=0; i<16; i++)
			{
				...
				CTimer::Get()->usDelay(12);
			}
			CTimer::Get()->usDelay(7);
		}
...
{
	uint16_t ret = 0;	// prh , dummy;
    int16_t n = w * h;
    uint8_t r, g, b;	// prh , tmp;
