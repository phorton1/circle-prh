...
void clear_common_interrupts()
{
	...
	dwc2_writel(GOTGINT, 0xffffffff);
	dwc2_writel(GINTSTS, 0xffffffff);
}
...
void initPeripheral()
{
	...
	dwc2_writel(GUSBCFG, usbcfg);                   // ends as   0x40001440 looks right
	resetPeripheral();
	...
	dwc2_writel(GAHBCFG,ahbcfg);			// ends as   0x00000000 write the zero back

	// Device Registers
	
	dwc2_writel(GRXFSIZ,					// starts as 0x00001000 == 4096
		pd->g_rx_fifo_sz);                  // we change 0x00000400 == 1024 
	dwc2_writel(GNPTXFSIZ,									// starts as 0x00201000 == 0x20 (flag) bytes at address 0x1000
	...
	dwc2_writel(GHWCFG2,cfg2);									// 0x228DD150 not changing anything!
	...
	for (int i = 1; i < pd->num_out_eps; i++)
	{
		...
		size |= addr;
		addr += size;
		dwc2_writel(DPTXFSIZN(ep_num),size);
	}

	// Set EP direction bits into GHWCFG1
	// dwc2_writel(GHWCFG1,0);
...
void CKernel::usbStuff(int what)
{
	if (what == 7)	// task
	{
		// read and display any waiting stuff
		
		u32 grxstsp = dwc2_readl(GRXSTSP);
		u32 ep_num = grxstsp & GRXSTS_EPNUM_MASK;
		u32 status = grxstsp & GRXSTS_PKTSTS_MASK;
		u32 bytes  = grxstsp & GRXSTS_BYTECNT_MASK;
		bytes >>= GRXSTS_BYTECNT_SHIFT;
		
		if (bytes)
		{
			printf("read(GRXSTSP) ep_num(%d),status(%08lx),bytes(%d)\n",
				   ep_num,status,bytes);
			
			// even tho it was someplace else, it shows in the EP0 fifo
			// let's try to read it.

			for (u16 i=0; i<bytes/4; i++)
			{
				u32 val = dwc2_readl(EPFIFO(0));
				printf("    got %08lx\n",val);
			}
