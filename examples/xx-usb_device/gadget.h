...
// I think they are mappings from little endian to platform specific big endian
...
#define __swab32(x)			(x)
#define __swab64(x)			(x)
#define __swab16p(p)		*(p)
#define __swab32p(p)		*(p)
#define __swab64p(p)		*(p)
...
enum usb_otg_state
	// prh from linux/include/usb/phy.h
{
...
enum usb_dr_mode {
	// prh - from linux/include/usb/otg.h
	USB_DR_MODE_UNKNOWN,
	USB_DR_MODE_HOST,
...
extern int dwc2_hsotg_pullup(struct dwc2_hsotg *hsotg, int is_on);
extern void dwc2_hsotg_core_connect(struct dwc2_hsotg *hsotg);

extern u32 dwc2_hsotg_irq(struct dwc2_hsotg *hsotg, int irq);
extern void dwc2_hsotg_dump(struct dwc2_hsotg *hsotg);

//-------------------------------------------
// prh - end of most of my changes
// though I may have munged anything below, as well
//-------------------------------------------
...
{
	return len;	// prh !g->quirk_ep_out_aligned_size ? len :
